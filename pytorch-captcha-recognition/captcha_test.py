...
def main():
    ...
    correct = 0
    total = 0
    for i, (images, labels, image_mark_name) in enumerate(test_dataloader):
        print('标注的名字：'+str(image_mark_name[0]))
        image = images
        ...
        if(predict_label == true_label):
            correct += 1
        if(total%200==0):
            print('1、Test Accuracy of the model on the %d test images: %f %%' % (total, 100 * correct / total))
    print('2、Test Accuracy of the model on the %d test images: %f %%' % (total, 100 * correct / total))
